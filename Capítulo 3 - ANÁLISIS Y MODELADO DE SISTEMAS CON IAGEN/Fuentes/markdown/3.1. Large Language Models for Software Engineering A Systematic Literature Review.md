...
### In the field of language processing, traditionalLanguage Models (LMs)have been foundational
...
### significant transition into the emergence ofLarge Language Models (LLMs)[ 526 , 558 ]. Equipped
...
Large Language Models for Software Engineering: A Systematic Literature Review 1:

#### Table 1. State-of-the-art surveys related to LLMs for SE.

```
Reference Year Scope of models^1 Scope of SE tasks SLR^2 Time frame # Collected Papers
Zhanget al.[543] 2023 Code LLM Automated program repair ✓ 2017-2023 185
Zhenget al.[564] 2023 Code LLM General SE scope ✓ 2021-2023 149
Fanet al.[85] 2023 LLM General SE scope × - Not specified
Zanet al.[526] 2023 LLM (12M+) NL2Code × 2020-2023 Not specified
Wanget al.[448] 2023 LLM (117M+) Software testing ✓ 2019-2023 52
Wanget al.[452] 2022 ML, DL^3 General SE scope ✓ 2009-2020 1,209 (ML) + 358 (DL)
Yanget al.[509] 2022 DL General SE scope ✓ 2015-2020 250
Watsonet al.[466] 2022 DL General SE scope ✓ 2009-2019 128
...
### This SLR follows the methodology proposed by Kitchenhamet al.[ 197 , 198 ], used in most other

### SE-related SLRs [ 229 , 261 , 352 , 452 ]. Following the guidelines provided by Kitchenhamet al., our
...
```
```
4,341 papers4,341 papers 5,078 papers
```
```
Filter out studies
with less than 8
...
```
```
1,192 papers 10,445 papers 65,290 papers 42,166 papers 85,671 papers 9,966 papers
```
```
IEEE Xplore ACM Digital Library Science Direct ScienceWeb of Springer arXiv
```
```
4,035 papers
```
...
### was published in 2017, we focused our search on papers published from that year onward^2. Two

### authors independently performed the automated search, and the search results from each database

### were merged and deduplicated. Specifically, we obtained 1,192 papers from IEEE Xplore, 10,
...
### system of 0, 1, 2, 3 (poor, fair, good, excellent). Finally, we calculated the total score of QAC4-QAC
...
```
```
PolyCoder (8) InstructGPT (5) CodeParrot (6) CodeGeeX (8)
```
```
PyCodeGPT (5) PanGu-Coder (1) OPT (5) StarCoder (25) Claude2 (2)
```
```
LaMDA (2)
```
...
### introduced by Panet al.[ 326 ] and categorize the mainstream LLMs investigated in this study
...
### example, in a collaborative effort with China Merchants Bank (CMB), Wanget al.[ 462 ] were able to
...
### Note that some papers use multiple datasets that span different categories, e.g., Xuet al.[ 493 ]
...
### from the 395 studies. We can find thatmost of the studies used text-based datasets, accounting
...
### tasks. Graph-based datasets are used in some research studies for SE tasks, e.g., Kolthoffet al.[ 203 ]
...
### or irrelevant text. For example, the dataset used by Leeet al.[ 216 ] was constructed from bug
...
### approximately 97.75% of the studies^5 .Specifically, code in tokens was widely adopted in 118
...
### structure and text in tokens in Niuet al.’s work [ 313 ]), showcased the potential for enhancing code
...
### instance deletion, anddata segmentation.
...
### task context to perform better. We observed that out of 83 studies, LLMs were fine-tuned usingfull

### fine-tuningtechniques to adapt to downstream SE tasks, with the majority being BERT series
...
### To reduce this computational burden, some researchers have previously usedIn-Context Learn-
...
### To address this problem, researchers have begun to applyParameter Efficient Fine-Tuning
...
### performance of LLMs in SE tasks. For instance, Panet al.[ 324 ] trained SteloCoder, specifically
...
### architecture to adjust the parameter count. Additionally, Silvaet al.[ 397 ] applied LoRA to LLaMA,
...
### three research works [ 267 , 461 , 570 ] utilized prompt tuning. For instance, Zhuet al.[ 570 ] proposed
...
### Luet al.[ 267 ] fine-tuned LLaMA-Reviewer for automating code review, while Wanget al.[ 461 ]
...
### tuning them on specific tasks without altering the original model’s parameters. Agarwalet al.[ 1 ]

### fine-tuned LLMs using adapter tuning techniques to make them suitable for code representation

### tasks. Wanget al.[ 447 ] indicated that LLMs refined through adapter tuning perform exceptionally
...
### LLMs in the LLM4SE domain, such asReinforcement Learning (RL)[ 156 , 157 , 161 , 403 , 504 ],

### Supervised Fine Tuning (SFT)[ 71 , 157 , 285 , 403 , 504 ], an unsupervised data augmentation method

### calledsyntax fine-tuning[ 345 ],knowledge preservation fine-tuning[ 395 ], andtask-oriented
...
### studies utilized few-shot prompting [ 7 , 91 , 95 , 104 , 185 , 469 , 494 , 550 ]. For instance, Genget al.[ 104 ]
...
### adopted zero-shot prompting [ 204 , 226 , 271 , 336 , 377 , 473 , 483 , 497 ]. For example, Liet al.[ 226 ]
...
### Chain-of-Thought (CoT) prompting.Weiet al.[ 468 ] introduced a prompting technique called
...
### thoughtful responses. Huanget al.[ 151 ] proposed a novel method leveraging the fault-tolerance
...
### code blocks. Tianet al.[ 429 ] also introduced the first test case-driven code generation technique,
...
### engineering methods, Zhouet al.[ 569 ] introduced an Automatic Prompt Engineer (APE) for
...
### automated prompts, Sunet al.[ 408 ] proposed a new prompt learning framework called PromptCS.
...
### snippets to guide the model’s code generation process. Huanget al.[ 144 ] proposed CodeCoT, and
...
### and desired task. Paranjapeet al.[ 327 ] introduced a framework, Automatic. ART, for generating
...
### overcome this limitation, Liet al.[ 222 ] proposed the Modular-of-Thought Coder (MoTCoder). They
...
### structural information, Liet al.[ 225 ] proposed SCoT prompting specifically for code generation
...
### to apply LLMs to SE tasks better. For instance, Renet al.[ 357 ] proposed a code generation method

### based on knowledge-driven prompt chains. Liet al.[ 233 ] applied differential prompting to ChaGPT

### to better identify test cases that cause failures in buggy programs. Ahmedet al.[ 7 ] enhanced the
...
### respectively, employing these metrics. For example, in the study conducted by Khanet al.[ 190 ],

### F1-score is utilized to evaluate the performance of an automatic bug-fixing model. Similarly,

### Sharmaet al.[ 383 ] use Precision and Recall to assess the effectiveness of a transformer-based model
...
### studies, respectively. For instance, Wanget al.[ 461 ] employed BLEU to evaluate a code-to-code

### translation model. Pass@k is used in the research by Jianget al.[ 170 ] to assess code generation
...
### reported SE tasks [ 509 ] addressed with LLMs, following the six phases of theSoftware Develop-
...
### they address (shown in Fig.10 (b)). The distribution reveals thatthe majority of studies, about
...
### can lead to suboptimal software artifacts during later development stages. Moharilet al.[ 291 ]

### and Ezziniet al.[ 83 ] have empirically demonstrated the significant role of LLMs such as BERT

### and SpanBERT in effectively addressing anaphoric ambiguity. Sridharaet al.[ 400 ] revealed that
...
### Heyet al.[ 135 ] employ BERT for requirement classification, where it excels in categorizing both

### FR and NFR requirements using a fine-tuning transfer learning technique, outstripping traditional

### methods. Luoet al.[ 271 ] introduce a BERT-based software requirement classification method,

### demonstrating remarkable transferability and generalization, especially in zero-shot scenarios.

### Requirements term identification.Moharilet al.[ 290 ] propose a technique for identifying terms
...
### cause confusion and affect comprehensibility. Wanget al.[ 462 ] offer a novel application of the
...
### and test cases, for product querying and development support [ 359 ]. Linet al.[ 246 ] found that
...
### GUI (Graphical User Interface) retrieval.Kolthoffet al.[ 203 ] present the application of BERT
...
### user needs. Whiteet al.[ 475 ] investigate the role of LLMs in augmenting this process. The study
...
### aging configurations and specifications becomes complex with larger systems. Mandalet al.[ 278 ]
...
Coder, PyCodeGPT, CodeParrot, GPT-2
```
```
HumanEval,
MBPP, MBCPP
```
```
Pass@k May 11, 2023 [224]
```
```
GPT-4 PaLM Coder, Codex, CodeGen-Mono, In-
coder, CodeGeeX, AlphaCode
```
```
HumanEval,
HumanEval-ET,
...
### human language descriptions. For example, Arakelyanet al.[ 19 ] rigorously evaluate the efficacy
...
### goals. Salzaet al.[ 373 ] explored the effectiveness of LLMs such as BERT [ 373 ] and RoBERTa [ 40 ] in
...
### Weiet al.[ 469 ] introduced CLEAR, an API recommendation method that employs the BERT sen-
...
### pendent of their lexical content. Recently, Zhanget al.[ 538 ] developed ToolCoder, which combines
...
### studies, Wanget al.[ 453 ] and Patilet al.[ 330 ] have both explored the potential of LLMs in this
...
### summarization [ 443 , 536 ]. Niuet al.[ 313 ] propose a novel sequence-to-sequence pre-training model
...
### and dependencies inherent in programming languages. Wanet al.[ 442 ] show through their research
...
### disconnect between code evolution and its accompanying documentation, Mastropaoloet al.[ 282 ]
...
### an application [ 363 ]. Zhuet al.[ 570 ] present AUMENA, a novel approach using the CodeT5 model
...
### workload and complexity. However, subjective estimates may introduce uncertainty. Fuet al.[ 98 ]
...
### Khanet al.[ 190 ] identified five API documentation smells and presented a benchmark of 1,000 API
...
### require labor-intensive manual rule creation or data labeling. Huanget al.[ 147 ] present an in-
...
### Code recommendation.Zhouet al.[ 567 ] pointed out that software developers tend to write
...
### code resources for various tasks. Rahmaniet al.[ 351 ] introduce a methodology to improve code
...
### in identifiers with the natural language vocabulary in other software artifacts. Zhanget al.[ 532 ]
...
### navigating the vast landscape of user-defined types. Addressing this complexity, Jesseet al.[ 165 ]
...
### the assistance of LLMs [35, 48, 424], Tanget al.[417] introduced novel approaches using LLMs to
...
### metrics, or stack trace analysis, which may not always provide precise results. Ciborowskaet al.[ 55 ]
...
### different operational contexts. Charalambouset al.[ 37 ] leverage LLMs, particularly the GPT-3.5, in
...
### a test oracle is often an undecidable problem [ 154 ]. Liet al.[ 232 ] investigated the application of
...
### code samples can have structural or semantic equivalence [ 414 ]. Sharmaet al.[ 383 ] investigate
...
### debugging process, such as AutoSD proposed by Kanget al.[ 183 ]. This model generates hypotheses

### about code problems and extracts relevant values to identify potential problems. In addition, the

### SELF-DEBUGGING method proposed by Chenet al.[ 45 ] enables LLM to debug its own generated
...
### by the success of the LLMs in natural language understanding, Fenget al.[ 91 ] propose AdbGPT,
...
### tify and diagnose bugs, performance bottlenecks, and other critical issues. Mastropaoloet al.[ 285 ]

### introduce LANCE, a system for automatically generating and injecting full log statements into

### Java code using the T5 model. Sridharaet al.[ 400 ] present that ChatGPT performs well in the log
...
### valuable insights into how users perceive and react to software applications. Zhanget al.[ 552 ]
...
### weaknesses in software applications. Pearceet al.[ 333 ] investigate how to use LLMs for software
...
### Bug prediction.Gomeset al.[ 110 ] conduct a BERT and TF-IDF (Term Frequency-Inverted Doc-
...
### amplifying the intricacy. Leeet al.[ 216 ] introduce the Light Bug Triage framework (LBT-P). This
...
### Zhanget al.[ 533 ] explored the potential of using k-shot learning with LLMs like GPT-3 to automate
...
### issues such as tag explosion. Heet al.[ 130 ] introduced PTM4Tag, a framework utilizing PLMs with
...
### resources, and manpower required to complete a software development project. Alhamedet al.[ 11 ]
...
### literature review. We additionally provide a replication package^6 for others to view.
...
```
### are preliminary studies on reducing code models’ size and improving their efficiency. Shiet al.[ 389 ]
...
### to inflated performance metrics [ 560 ]. For instance, Brownet al.[ 29 ] discovered a code bug that
...
### preserving transformations. For example, Yanget al.[ 510 ] show that the performance of CodeBERT
...
### 93 , 126 , 334 ]. For instance, Ferraget al.[ 93 ] showcased the efficacy of LLMs in cyber reasoning
...
### the models themselves [ 60 , 81 , 258 , 353 , 354 , 481 ]. As an example, Wuet al.[ 481 ] delve into methods
...
Shepherd. 2020. Software documentation: the practitioners’ perspective. InProceedings of the ACM/IEEE 42nd
...
Decoding of Code LMs with Static Analysis of Repository Context. InThirty-seventh Conference on Neural Information
Processing Systems.
...
```
2023. Extending Source Code Pre-Trained Language Models to Summarise Decompiled Binarie. In2023 IEEE
...
SemanticCloneBench. In2023 IEEE International Conference on Software Maintenance and Evolution (ICSME). IEEE,
1–13.
[11]Mohammed Alhamed and Tim Storer. 2022. Evaluation of Context-Aware Language Models and Experts for Effort
Estimation of Software Maintenance Issues. In2022 IEEE International Conference on Software Maintenance and
Evolution (ICSME). IEEE, 129–138.
[12] Frances E Allen. 1970. Control flow analysis.ACM Sigplan Notices5, 7 (1970), 1–19.
...
abstract syntax trees. InProceedings. International Conference on Software Maintenance (Cat. No. 98CB36272). IEEE,
368–377.
...
analysis in the software engineering domain using bert. In2020 IEEE International conference on software maintenance
...
coverage measures via execution logs. InProceedings of the 33rd ACM/IEEE International Conference on Automated
...
models for low-resource programming languages. InProceedings of the 30th IEEE/ACM International Conference on
...
tests using log-recovered workloads at varying granularities of user behaviour. In2019 34th IEEE/ACM International
...
API-enhanced AST. InProceedings of the 16th ACM International Conference on Computing Frontiers. 174–182.
...
domain-specific languages.Advances in Neural Information Processing Systems34 (2021), 22196–22208.
...
```
2022. Using a Nearest-Neighbour, BERT-Based Approach for Scalable Clone Detection. In2022 IEEE International
...
[54]Agnieszka Ciborowska and Kostadin Damevski. 2022. Fast changeset-based bug localization with BERT. InProceedings
...
completion.IEEE Transactions on Software Engineering48, 12 (2021), 4818–4837.
...
are Zero-Shot Fuzzers: Fuzzing Deep-Learning Libraries via Large Language Models. InProceedings of the 32nd ACM
...
recommendation systems for software engineering: the CROSSMINER experience.Empirical Software Engineering26,
4 (2021), 69.
...
ambiguity in requirements: a multi-solution study. InProceedings of the 44th International Conference on Software
Engineering. 187–199.
...
discussions.Journal of Systems and Software209 (2024), 111935.
[95]Emily First, Markus Rabe, Talia Ringer, and Yuriy Brun. 2023. Baldur: Whole-proof generation and repair with large
language models. InProceedings of the 31st ACM Joint European Software Engineering Conference and Symposium on
...
IEEE Transactions on Software Engineering49, 2 (2022), 611–625.
...
mation retrieval methods to improve traceability recovery. In2011 27th IEEE International Conference on Software
...
and Software Technology135 (2021), 106566.
...
long-lived bug prediction in FLOSS: a comparative study.Information and Software Technology160 (2023), 107217.
...
on Software Engineering and Methodology32, 3 (2023), 1–57.
...
[116]Xiaodong Gu, Hongyu Zhang, and Sunghun Kim. 2018. Deep code search. InProceedings of the 40th International
Conference on Software Engineering. 933–944.
[117]Xiaodong Gu, Hongyu Zhang, Dongmei Zhang, and Sunghun Kim. 2016. Deep API learning. InProceedings of the
...
the potential of chatgpt in automated code refinement: An empirical study. InProceedings of the 46th IEEE/ACM
...
study. InProceedings of the 11th working conference on mining software repositories. 352–355.
...
with mobile analytics: An empirical study on firebase. In2021 IEEE/ACM 8th International Conference on Mobile
...
recommendation of stack overflow posts with pre-trained models. InProceedings of the 30th IEEE/ACM International
...
in-the-loop system for dockerfile repair. In2021 IEEE/ACM 43rd International Conference on Software Engineering
(ICSE). IEEE, 1148–1160.
[135]Tobias Hey, Jan Keim, Anne Koziolek, and Walter F Tichy. 2020. Norbert: Transfer learning for requirements
classification. In2020 IEEE 28th International Requirements Engineering Conference (RE). IEEE, 169–179.
...
Mona Attariyan, and Sylvain Gelly. 2019. Parameter-efficient transfer learning for NLP. InInternational Conference on
...
[143]Xing Hu, Ge Li, Xin Xia, David Lo, and Zhi Jin. 2018. Deep code comment generation. InProceedings of the 26th
...
worrying about the task-API knowledge gap. InProceedings of the 33rd ACM/IEEE International Conference on
...
Crashtranslator: Automatically reproducing mobile application crashes directly from stack trace. InProceedings of the
...
Duplicate bug report detection by using sentence embedding and fine-tuning. In2021 IEEE international conference on
...
neural attention model. In54th Annual Meeting of the Association for Computational Linguistics 2016. Association for
...
learning from structure and naming sequences. InProceedings of the 44th International Conference on Software
Engineering. 401–412.
...
Transactions on Software Engineering49, 4 (2022), 1508–1522.
...
relation and transfer expansion.IEEE Transactions on Software Engineering48, 2 (2020), 519–537.
...
Advances in Neural Information Processing Systems35 (2022), 11785–11799.
[177]Robbert Jongeling, Subhajit Datta, and Alexander Serebrenik. 2015. Choosing your weapons: On sentiment analysis
tools for software engineering research. In2015 IEEE international conference on software maintenance and evolution
(ICSME). IEEE, 531–535.
...
embedding of source code. InInternational conference on machine learning. PMLR, 5110–5121.
...
api documentation smells: Practitioners’ perspectives. In2021 IEEE International Conference on Software Analysis,
...
documentation of r packages. In2022 IEEE International Conference on Software Analysis, Evolution and Reengineering
(SANER). IEEE, 765–776.
...
reporting secondary studies.IEEE Transactions on Software Engineering49, 3 (2022), 1273–1298.
[199]Eric Knauss, Siv Houmb, Kurt Schneider, Shareeful Islam, and Jan Jürjens. 2011. Supporting requirements engineers in
recognising security issues. InRequirements Engineering: Foundation for Software Quality: 17th International Working
...
are zero-shot reasoners.Advances in neural information processing systems35 (2022), 22199–22213.
[203]Kristian Kolthoff, Christian Bartelt, and Simone Paolo Ponzetto. 2023. Data-driven prototyping via natural-language-
based GUI retrieval.Automated Software Engineering30, 1 (2023), 13.
...
Wang, and Tao Yu. 2023. DS-1000: A natural and reliable benchmark for data science code generation. InInternational
...
transformer (GPT-2). InProceedings of the Third International Workshop on Automated Program Repair. 61–68.
...
Quyet Thang Huynh. 2022. Autopruner: transformer-based call graph pruning. InProceedings of the 30th ACM Joint
...
Language Model. InProceedings of the 37th IEEE/ACM International Conference on Automated Software Engineering.
1–11.
...
Lin Liu. 2014. Non-functional requirements as qualities, with a spice of ontology. In2014 IEEE 22nd International
...
acceptance and ranking models. In2021 IEEE International Conference on Software Maintenance and Evolution (ICSME).
IEEE, 195–205.
...
with pre-trained models.ACM Transactions on Software Engineering and Methodology32, 6 (2023), 1–22.
...
Technology88 (2017), 67–95.
[230]Lingwei Li, Li Yang, Huaxi Jiang, Jun Yan, Tiejian Luo, Zihan Hua, Geng Liang, and Chun Zuo. 2022. AUGER:
automatically generating review comments with pre-training models. InProceedings of the 30th ACM Joint European
...
are the key: Unlocking chatgpt to find failure-inducing tests with differential prompting. In2023 38th IEEE/ACM
...
Integrating Semantic Features and Expert Features for Software Effort Estimation. InProceedings of the 46th IEEE/ACM
...
Testing and repairing code completion systems. In2023 IEEE/ACM 45th International Conference on Software Engineering
(ICSE). IEEE, 1238–1250.
...
[245]Yuding Liang and Kenny Zhu. 2018. Automatic generation of text descriptive comments for code blocks. InProceedings
...
more accurate links with pre-trained bert models. In2021 IEEE/ACM 43rd International Conference on Software
...
completion. InProceedings of the 35th IEEE/ACM International Conference on Automated Software Engineering. 473–485.
...
Information Processing Systems35 (2022), 1950–1965.
...
repositories. InProceedings of the 33rd ACM/IEEE International Conference on Automated Software Engineering. 282–
292.
...
systematic literature review.Comput. Surveys55, 8 (2022), 1–36.
...
Context-aware automated text input generation for mobile gui testing. In2023 IEEE/ACM 45th International Conference
...
[269]James H Lubowitz. 2023. ChatGPT, an artificial intelligence chatbot, is impacting medical literature.Arthroscopy39, 5
(2023), 1121–1122.
...
Classification using BERT-based Pretrained Language Models. InProceedings of the 37th IEEE/ACM International
...
KnowLog: Knowledge Enhanced Pre-trained Language Model for Log Understanding. InProceedings of the 46th
...
code search. InProceedings of the 31st ACM Joint European Software Engineering Conference and Symposium on the
...
comment completion. In2021 IEEE International Conference on Software Maintenance and Evolution (ICSME). IEEE,
159–170.
...
Admitted Technical Debt: How Far Are We?. In2023 38th IEEE/ACM International Conference on Automated Software
...
statements. InProceedings of the 44th International Conference on Software Engineering. 2279–2290.
...
learning. InProceedings of the 1st International Workshop on Natural Language-based Software Engineering. 51–58.
...
(2023).
[293]Robert C Moore and William Lewis. 2010. Intelligent selection of language model training data. InProceedings of the
...
and Danny Dig. 2016. API code recommendation using statistical learning from fine-grained changes. InProceedings
...
java projects. In2017 IEEE/ACM 39th International Conference on Software Engineering Companion (ICSE-C). IEEE,
164–166.
...
code search.IEEE Transactions on Services Computing9, 5 (2016), 771–783.
...
pre-training for learning source code representations. InProceedings of the 44th International Conference on Software
...
ASTs for Code Completion. In2023 IEEE International Conference on Software Analysis, Evolution and Reengineering
(SANER). IEEE, 834–844.
...
zero-shot vulnerability repair with large language models. In2023 IEEE Symposium on Security and Privacy (SP). IEEE,
2339–2356.
...
prompts with fix templates for repairing python type errors. InProceedings of the 46th IEEE/ACM International
...
machine learning.IEEE Transactions on Software Engineering48, 12 (2021), 5050–5067.
...
Machine Learning Research21, 1 (2020), 5485–5551.
...
software engineering.IEEE Transactions on Software Engineering45, 8 (2018), 760–781.
...
Code Generation with Knowledge-Driven AI Chaining. In2023 38th IEEE/ACM International Conference on Automated
...
large language models for the environment.Environmental Science & Technology57, 9 (2023), 3464–3466.
[361]Martin P Robillard. 2009. What makes APIs hard to learn? Answers from developers.IEEE software26, 6 (2009),
27–34.
...
software?. In2012 34th International Conference on Software Engineering (ICSE). IEEE, 255–265.
...
technique for debugging complex distributed systems. InProceedings of the 5th workshop on ACM SIGOPS European
...
Learning for Code Search. In2022 IEEE International Conference on Software Maintenance and Evolution (ICSME). IEEE,
94–105.
...
Vulnerability Detection. InProceedings of the 46th IEEE/ACM International Conference on Software Engineering. 1–13.
...
for neural code search. InProceedings of the 44th International Conference on Software Engineering. 1609–1620.
[414]Jeffrey Svajlenko, Judith F Islam, Iman Keivanloo, Chanchal K Roy, and Mohammad Mamun Mia. 2014. Towards a big
data curated benchmark of inter-project code clones. In2014 IEEE International Conference on Software Maintenance
...
Transformer-based language models for software vulnerability detection. InProceedings of the 38th Annual Computer
...
Evaluating representation learning of code changes for predicting patch correctness in program repair. InProceedings
...
Features for Accurate Prediction of Correct Patches.ACM Transactions on Software Engineering and Methodology32,
4 (2023), 1–34.
...
2022. Using pre-trained models to boost code review automation. InProceedings of the 44th International Conference
...
natural language processing in the software engineering domain.IEEE Transactions on Software Engineering49, 4
(2022), 1487–1507.
...
network learning for semantic source code retrieval. In2019 34th IEEE/ACM International Conference on Automated
...
You See What I Want You to See: Poisoning Vulnerabilities in Neural Code Search. InProceedings of the 30th
...
source code summarization via deep reinforcement learning. InProceedings of the 33rd ACM/IEEE international
...
Xiaoguang Mao. 2023. Natural Language to Code: How Far Are We?. InProceedings of the 31st ACM Joint European
...
on Software Engineering49, 3 (2022), 1188–1231.
...
flow-augmented abstract syntax tree. In2020 IEEE 27th International Conference on Software Analysis, Evolution and
...
codet5 for automatic program repair. InProceedings of the 31st ACM Joint European Software Engineering Conference
...
detection in natural language requirements. In2020 IEEE 28th International Requirements Engineering Conference (RE).
IEEE, 180–191.
[463]Yawen Wang, Junjie Wang, Hongyu Zhang, Xuran Ming, Lin Shi, and Qing Wang. 2022. Where is your app frustrating
users?. InProceedings of the 44th International Conference on Software Engineering. 2427–2439.
...
and syntactical information in source code.. InIJCAI. 3034–3040.
...
api recommendation. InProceedings of the 44th International Conference on Software Engineering. 376–387.
...
with completion engines for automated program repair. InProceedings of the 31st ACM Joint European Software
...
and Understanding of Big Code for AI-Assisted Programming: A Review.Entropy25, 6 (2023), 888.
[479]Di Wu, Yang Feng, Hongyu Zhang, and Baowen Xu. 2024. Automatic recognizing relevant fragments of APIs using
API references.Automated Software Engineering31, 1 (2024), 3.
...
Pre-Trained Language Models. InProceedings of the 45th International Conference on Software Engineering (ICSE ’23).
...
language models of code. InProceedings of the 6th ACM SIGPLAN International Symposium on Machine Programming.
1–10.
[494]Junjielong Xu, Ziang Cui, Yuan Zhao, Xu Zhang, Shilin He, Pinjia He, Liqun Li, Yu Kang, Qingwei Lin, Yingnong
Dang, et al.2024. UniLog: Automatic Logging via LLM and In-Context Learning. InProceedings of the 46th IEEE/ACM
International Conference on Software Engineering. 1–12.
...
Abilities of ChatGPT. In2023 38th IEEE/ACM International Conference on Automated Software Engineering (ASE). IEEE,
1887–1898.
...
fault localization. InProceedings of the 46th IEEE/ACM International Conference on Software Engineering. 1–12.
...
Aspect-based api review classification: How far can pre-trained transformer model go?. In2022 IEEE International
...
code snippets. InProceedings of the 13th International Conference on Mining Software Repositories. 391–402.
...
in systematic literature reviews: A software engineering perspective.Information and Software Technology130 (2021),
106397.
...
[510]Zhou Yang, Jieke Shi, Junda He, and David Lo. 2022. Natural Attack for Pre-Trained Models of Code. InProceedings
...
Ability under New Log Types. InProceedings of the 31st ACM Joint European Software Engineering Conference and
...
2022. CIRCLE: Continual repair across programming languages. InProceedings of the 31st ACM SIGSOFT International
...
Large Language Models Meet NL2Code: A Survey. InProceedings of the 61st Annual Meeting of the Association for
...
and Software Technology53, 6 (2011), 625–637.
...
models to resolve textual and semantic merge conflicts (experience paper). InProceedings of the 31st ACM SIGSOFT
...
source code and natural language editing. InProceedings of the 37th IEEE/ACM International Conference on Automated
...
summarization. InProceedings of the ACM/IEEE 42nd International Conference on Software Engineering. 1385–1397.
[537]Jian Zhang, Xu Wang, Hongyu Zhang, Hailong Sun, Kaixuan Wang, and Xudong Liu. 2019. A novel neural source code
representation based on abstract syntax tree. In2019 IEEE/ACM 41st International Conference on Software Engineering
(ICSE). IEEE, 783–794.
...
template-based automated program repair via mask prediction. In2023 38th IEEE/ACM International Conference on
...
analysis for software engineering: How far can pre-trained transformer models go?. In2020 IEEE International
...
reviewer reranking for code generation. InInternational Conference on Machine Learning. PMLR, 41832–41846.
...
[567]Shufan Zhou, Beijun Shen, and Hao Zhong. 2019. Lancer: Your code tell me what you need. In2019 34th IEEE/ACM
...
Data. In2022 IEEE 33rd International Symposium on Software Reliability Engineering (ISSRE). IEEE, 446–457.
...
suites/cases
```
##### 17 [ 72 ] [ 89 ] [ 120 ] [ 145 ] [ 144 ] [ 163 ] [ 240 ] [ 296 ] [ 316 ] [ 322 ]
...
tions
```
##### 8 [66] [70] [124] [192] [205] [336] [371] [428]
...
prompting
```
##### 18 [ 63 ] [ 91 ] [ 151 ] [ 150 ] [ 233 ] [ 225 ] [ 263 ] [ 296 ] [ 346 ] [ 378 ] [ 394 ] [ 429 ]
...
```
```
9 [11] [79] [386] [418] [426] [449] [502] [499] [545]
```
```
ROC (Receiver Op-
erating Characteris-
...
```
```
15 [ 49 ] [ 54 ] [ 86 ] [ 160 ] [ 223 ] [ 242 ] [ 221 ] [ 246 ] [ 270 ] [ 281 ] [ 351 ] [ 373 ] [ 390 ]
[447] [469]
...
```
```
62 [ 1 ] [ 7 ] [ 9 ] [ 19 ] [ 40 ] [ 44 ] [ 56 ] [ 66 ] [ 102 ] [ 104 ] [ 115 ] [ 157 ] [ 156 ] [ 164 ] [ 170 ]
